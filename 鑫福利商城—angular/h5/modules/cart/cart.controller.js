(function () {
	...
	CartController.$inject = ['$scope', '$http', '$location', '$state','AppAuthenticationService', 'ConfirmCartService', 'CartModel' , 'ENUM'];

	function CartController($scope, $http, $location, $state, AppAuthenticationService, ConfirmCartService, CartModel,  ENUM) {
			...
			if (isSelectedAll) {
				for (var i = 0; i < groups.length; ++i) {
					...
					for (var j = 0; j < group.goods.length; ++j) {
						...
						if(group.goods[j].product.is_real==1 && group.goods[j].product.suppliers == ENUM.suppliers.id){
							if(group.goods[j].is_buy != 'undefined' && group.goods[j].is_buy>0){
