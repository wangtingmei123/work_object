(function () {
  ...
  angular
  .module('app')
  .controller('ProductcsController', ProductcsController);

  ProductcsController.$inject = ['$scope', '$http', '$window', '$timeout', '$location', '$state', '$stateParams', 'API', '$rootScope','ENUM', 'AppAuthenticationService', 'ConfirmProductService', 'CartModel','CONSTANTS','ConfigModel'];

  function ProductcsController($scope, $http, $window, $timeout, $location, $state, $stateParams, API,$rootScope, ENUM, AppAuthenticationService, ConfirmProductService, CartModel,CONSTANTS,ConfigModel) {
      ...
      sessionStorage.setItem("productId", productId);//保存点击的状态
      ...
      if($scope.user==undefined){
        ...
        $scope.use_toc=$scope.user.company.id
      }

      console.log($scope.use_toc)

      
      console.log(sessionStorage.getItem("show_toc"))
      sessionStorage.setItem("show_toc", true);//保存点击的状态
...
// 指定商品数量限制
   ...
   //场馆预定
      ...
      function _initConfig(wechat,url){
        ...
        var shared_link='http://shop.xindongguoji.com/h5/?#/productcs/?product='+$scope.sgproduct_id_c1
